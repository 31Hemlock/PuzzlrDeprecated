...
class DragControls extends EventDispatcher {
	...
	constructor( _objects, _camera, _domElement ) {
		...
		function onPointerMove( event ) {
			...
			if ( event.pointerType === 'mouse' || event.pointerType === 'pen' ) {
				...
				if ( _intersections.length > 0 ) {
                    ...
                    // _plane.setFromNormalAndCoplanarPoint(up, _worldPosition.setFromMatrixPosition(_selected.matrixWorld));
		...
		function onPointerDown( event ) {
            if (event.button == 1) return; // Middle mouse button was pressed; do nothing.
			...
			if ( _intersections.length > 0 ) {
				if (_intersections[0].object.parent?.type === "Scene") {
				...
				// if selected object is a mesh, make it = intersections[0].object
